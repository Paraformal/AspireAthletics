    ...
    <section className=' w-full min-h-screen'>
      <div className=' max-container padding-hero-y padding-x'>
        <div  className='flex justify-between items-end mb-28 max-md:flex-col max-md:items-start max-md:gap-5 max-md:mb-20'>
          <div>
            <p className=' text-[#9D101C] relative before:absolute before:w-20 before:h-1 before:bg-[#9D101C] before:top-[50%] before:left-0 pl-24 text-2xl before:translate-y-[-50%]'>OUR TEAM MEMBERS</p>
          ...
          <div>
            <button className=' py-4 px-9 text-xl group relative text-white bg-[#9D101C] rounded-sm'>
            ...
            {teamMembers.map((val)=>(
              <div key={val.name} className='group relative overflow-hidden'>
                ...
                <div className='absolute bottom-[-50px] duration-[.4s] group-hover:bottom-0 left-0 w-full group-hover:bg-[red] p-7 pb-8'>
                  <p className=' font mb-2 text-gray-300 relative before:absolute before:w-10 before:h-1 before:bg-[#9D101C] before:top-[50%] before:left-0 pl-14 text-lg before:translate-y-[-50%] group-hover:text-white group-hover:before:bg-white'>{val.role}</p>
