    ...
    <section className=' w-full min-h-screen'>
      <div className=' flex justify-between items-center gap-20 max-container padding-hero-y relative max-lg:flex-col padding-x'>
        <div className=' relative flex-1 flex justify-center items-center max-lg:w-full'>
          ...
          <RiDoubleQuotesL className=' absolute top-[50%] translate-y-[-50%] -right-[75px] text-[#9D101C] text-[180px] max-lg:hidden'/>
        </div>

        <div className=' flex-1'>
          <p className=' text-[#9D101C] relative before:absolute before:w-20 before:h-1 before:bg-[#9D101C] before:top-[50%] before:left-0 pl-24 text-2xl before:translate-y-[-50%]'>CLIENT FEEDBACK</p>
        ...
        <div className=' flex relative'>
          ...
          <div className=' flex text-4xl mt-20 justify-start max-lg:mt-5'>
            {client.map((val,index)=>(
              <div key={index} onClick={()=>dotClick(index)} className={` cursor-pointer ${currSlide === index ? " text-[#9D101C]" : "text-4xl"}`}>
                <BsDot />
              </div>
            ))}
          </div>
        ...
        <div className=' absolute left-4 max-lg:top-[25%] h-10 w-10 flex justify-center items-center max-lg:left-1 max-lg:-translate-y-[-50%] text-4xl max-lg:text-2xl text-[#9D101C] cursor-pointer rounded-full hover:bg-black hover:text-white duration-300 max-sm:top-[460px] max-sm:left-32 max-sm:rounded-full' onClick={prevSlide}>
        <IoIosArrowBack />
        </div>

        <div className=' absolute right-4 max-lg:top-[25%] h-10 w-10 flex justify-center items-center max-lg:right-1 max-lg:-translate-y-[-50%] text-4xl max-lg:text-2xl text-[#9D101C] cursor-pointer rounded-full hover:bg-black hover:text-white duration-300 max-sm:top-[460px] max-sm:right-32' onClick={nextSlide}>
          <IoIosArrowForward />
        </div>
      </div>
