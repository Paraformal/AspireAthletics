  ...
  return (
    <section id='blog' className=' w-full'>
      <div className=' max-container padding-hero-y padding-x'>
        <p className=' text-[#9D101C] relative before:absolute before:w-20 before:h-1 before:bg-[#9D101C] before:top-[50%] before:left-0 pl-24 text-2xl before:translate-y-[-50%] text-center w-fit m-auto max-sm:before:w-16 max-sm:pl-20'>RECENT NEWS FORM BLOG</p>
          ...
          {blog.map((val)=>(
            <div key={val.img} className='group'>
              <div className=' flex justify-center items-center relative overflow-hidden '>
                 ...
                 <p className='absolute bottom-0 left-0 text-2xl bg-[#9D101C] text-white p-6 max-lg:p-4'>{val.date} Nov</p>
              </div>
              ...
              <div className=' p-8 max-lg:p-5'>
                ...
                <p className=' cursor-pointer hover:text-[#9D101C] font text-2xl font-bold w-[70%] max-lg:w-[100%] max-lg:text-lg'>
                  {val.desc}
                </p>
              </div>
            </div>
